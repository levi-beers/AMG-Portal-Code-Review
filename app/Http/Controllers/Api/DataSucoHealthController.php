...
use AMGPortal\Http\Requests\Data\CreateNewDataSucoHealthSubscriber;
use AMGPortal\Http\Resources\DataSucoHealthResource;
use AMGPortal\Http\Controllers\Controller;
use AMGPortal\DataSucoHealth;


class DataSucoHealthController extends Controller
{
     ...
     * @param CreateNewDataSucoHealthSubscriber $request
     * @return UserResource
     */
    public function store(CreateNewDataSucoHealthSubscriber $request)
    {
        //print "testtesttest";
        $data = $request->only([
...
//        ];
        ...
        $user = DataSucoHealth::create($data);

        return new DataSucoHealthResource($user);
